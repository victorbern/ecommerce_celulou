...
export class UpdateProdutoController {
    ...
    async handle(request: Request, response: Response, next: NextFunction): Promise<Response> {
        ...
        const { valor, nomeProduto, marca, descricaoProduto, pesoGramas, alturaCM, larguraCM, comprimentoCM, categorias } = request.body;
        try {
            let result = await this.updateProdutoUC.execute({
                codigoProduto: codigo, valor, nomeProduto, marca, descricaoProduto, pesoGramas, alturaCM, larguraCM, comprimentoCM, categorias
            });
